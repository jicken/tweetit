...
public class TweeterViewImpl extends ReverseCompositeView<TweeterPresenter> implements TweeterView {
    ...
    @UiHandler("post")
    public void post(ClickEvent click) {
        TweetDTO dto = tweeter.getEditorDriver().flush();
        Set<ConstraintViolation<TweetDTO>> errors = validator.validate(dto);
        if (!errors.isEmpty()) {
            tweeter.getEditorDriver().setConstraintViolations(new ArrayList<ConstraintViolation<?>>(errors));
        } else {
            getPresenter().postTweet(dto);
        }
    }
}
